'''
This module is intended to control an LED array inside of a cryostat to be used for LED mapping of an MKID wafer.
The module should be run using an Arduino. While any Arduino should work, the module was written using an Arduino
Nano Every. This module uses the Telemetrix package to control the Arduino over a serial connection, which requires
a Telemetrix script to be run from the Arduino IDE. Then, creating an instance of the Telemetrix class causes the
program to locate the Arduino board. This module also uses the telemetrix_pca9685 package to connect to all PCA9685
IC chips over an I2C connection.

All connections are automatically set up when an instance of the Arduino class is created using a configuration text
file. The Arduino class's public methods should be sufficient for controlling the LED array. In case more manual
debugging is needed, the get_attributes method returns all attributes of the Arduino object, including private
attributes.
...
class Arduino:
    ...
    Attributes:
        ...
        __drivers   (1D array)       - contains all TelemetrixPCA9685 objects (Adafruit driver representations)
                                       which were initialized along with the Arduino board
        ...
        _LEDs       (3D array)       - axes 0 and 1 are analogous to the LED array being controlled (axis 0 is
                                       ...
                                       - cathode_pin is the Arduino board's pin number which the LED uses as its cathode
                                       - anode_channel is the driver's channel number which the LED uses as its anode
    ...
    Methods:
        ...
        _pin_mode      (internal) - directly changes an Arduino board pin to digital input, digital output,
                                    or analog output mode
        ...
        get_attributes (public)   - returns all attributes as a dictionary, including private attributes
    '''
    ...
    def __init__(self, file_path):
        ...
        Parameters:
            ...
            file_path (str)  - file path for the configuration file which dictates the LED array's setup;
        ...
        self.__drivers = np.array([Driver(board=self.__board, i2c_address=address) for address in self.__addresses])
    ...
    def __LED_array(self, config):
        ...
        Parameters:
            ...
            config (2D array) - result of np.loadtxt call on the configuration file;
                                data must be formatted as axis 0 distinguishing between different pins/channels,
                                axis 1 as [row_or_col_number, pin_or_channel_number, driver_address];
                                a pin has no driver address, so driver_address should be 0 for a pin
    ...
    def _pin_mode(self, pin, mode):
        ...
        '''
        Directly changes an Arduino board pin to digital input, digital output, or analog output mode.
        These are equivalent to read, write, and pwm modes in the Arduino IDE.
        ...
        if mode == 'READ':
            ...
            raise Exception('Invalid mode; mode must be \'READ\', \'WRITE\', or \'PWM\'')
    ...
    def LED_on(self, i, j=None, bright=1):
        ...
        LED's cathode pin and anode channel. Does not make assumptions about a pin or channel's state.
        Can either be called with an LED number or the LED's coordinates in the LED array.
        ...
        Parameters:
            ...
            i      (int)               - either the 0-indexed row coordinate of the desired LED,
                                         ...
                                         then downward (ex.: LED 6 in a 3x3 array is the rightmost LED in the middle row)
            ...
            bright (iterable or float) - if iterable, then [on, off] parameters for _channel_out method;
                                         if float, then creates [on, off] parameters as [0, int(4095 * cycle)],
                                         so float must be between 0 and 1 inclusive and describes LED brightness;
                                         default value 1 corresponds to LED always on for max brightness
    ...
    def LED_blink(self, i, j=None, num_iter=0, period=0, bright=1):
        ...
        Parameters:
            ...
            bright   (iterable or float) - bright parameter for LED_on;
                                           default value 1 corresponds to turning the LED on at full brightness
    ...
    def LED_off(self, i, j=None):
        ...
        Parameters:
            ...
            i     (int)  - either the 0-indexed row coordinate of the desired LED,
                           or the desired LED's LED number;
                           LED numbers are 0-indexed and count from the upper left corner left-to-right then downward
                           (ex.: LED 6 in a 3x3 array is the rightmost LED in the middle row)
    ...
    def global_off(self):
        ...
        '''
        Calls LED_off for every LED, thus never turning on any channels in their cycle and pushing all
        pins to read mode with a write-ready low voltage.
